...
<!-- Generated from src/Swipewalk.Core/Limitations/KnownLimitations.cs.
     Regenerate: dotnet run --project src/Swipewalk.Cli -- limitations > docs/limitations.md -->

Swipewalk finds some accessibility issues automatically. This page lists what it cannot check or may get wrong, and what to test by hand instead. Each scan report shows the entries that apply to that scan.

- **Limitation**: something Swipewalk cannot check or may get wrong.
- **Framework note**: platform or framework behavior that affects accessibility.

## All platforms · All frameworks

### Automated checks cover only part of WCAG

`partial-wcag-coverage` · Limitation · Coverage

- **What:** Each check tests only part of the WCAG criteria it maps to. Most WCAG 2.2 success criteria are not tested automatically at all. On Android, when the instrumentation harness ran (see "android-atf-harness"), Google's Accessibility Test Framework adds its own checks (14 in ATF 4.1.1, some overlapping Swipewalk's own rules); one of them, UnexposedTextCheck, needs text recognition the harness does not currently supply and never produces a finding. Another, TextSizeCheck, reported a finding on a physical Pixel 4a (Android 13) but not on an Android 16 emulator scanning the exact same px-sized text (samples/NativeAndroid, 2026-09-23); the two also differ in device type, density and screen size, so which of those differences explains it hasn't been pinned down. These are partial too.
...
- **Check manually:** Swipe through the screen with TalkBack, VoiceOver or Narrator and compare with the predicted transcript.
- **Planned:** Screen-reader testing

### Behavior over time is not checked

`no-interaction-checks` · Limitation · Coverage

- **What:** Scans are a snapshot. Announcements of status messages, focus movement after actions, timeouts, animations and gestures are not tested.
...
- **Check manually:** Check flagged and image-backed text or icons with a color picker, and check focus indicators and input borders for 3:1 contrast.

### Decorative and informative images look the same

`decorative-images` · Limitation · Rules · rules: `missing-name`

- **What:** The accessibility trees used do not say whether an image was hidden on purpose, so every image without a text alternative is listed for review, including decorative ones.
- **Impact:** Decorative images appear under Needs review even when they are correctly hidden.
- **Check manually:** For each listed image, decide whether it conveys information. If it does, it needs a description.

### Images inside a named control or row are not listed

`images-in-named-rows` · Limitation · Rules · rules: `missing-name`

- **What:** An image inside a control or row that screen readers read as one element with a name (for example a menu row) is not reported separately, because screen readers announce the row's name instead.
- **Impact:** An icon that conveys information not in the row's name, such as a warning or status icon, is not flagged.
- **Check manually:** In lists and menus, check with a screen reader that icons carrying meaning are part of what is announced.

### Identifier-like names are found by pattern

`identifier-heuristic` · Limitation · Rules · rules: `identifier-name`

- **What:** Names are flagged when they look like code (snake_case, prefixes like btn/img, or equal to the AutomationId). Meaningless names that look like words are not caught.
- **Impact:** Names such as "Button 3" or "Image" are not flagged; unusual real words with underscores might be.
- **Check manually:** Read the predicted transcript: every name should describe the element's purpose.

### Input-purpose candidates are found by keyword, not by a declared hint

`input-purpose-heuristic` · Limitation · Rules · rules: `input-purpose`

- **What:** Swipewalk flags a text field as a WCAG 1.3.5 candidate only when its label, hint or identifier text matches a small, specific set of phrases (for example "first name", "email", "street address") -- not the bare word "name" alone, which is too common on non-personal fields ("file name", "team name"). Neither Android's accessibility tree (uiautomator dump or the instrumentation harness) nor iOS's XCUITest exposes whether the app actually declared an autofill/content-type hint for a field (Android autofillHints, iOS textContentType), so the check can never confirm one either way -- only guess the purpose from visible text.
...
- **Check manually:** For each conditional clause in the report, decide from the app's real features whether it applies, then follow its guided steps or the standard's own text.
...
### Results reflect the rule versions listed in the report

`rule-versions` · Limitation · Coverage

- **What:** Rules and standards mappings are checked against specific versions of WCAG, laws and platform guidelines, listed in each report with the date the mapping was reviewed. Swipewalk does not detect changes to them automatically.
...
### Scans use the device's current settings unless told to check the alternative too

`device-settings` · Limitation · Coverage

- **What:** Each scan captures the screen as the device is. `scan --appearance both` captures the screen in the device's other dark/light appearance too and runs every check on it (Android `cmd uimode night`; iOS Simulator `simctl ui appearance`; a physical iPhone through the harness driving Settings > Appearance -- skipped with a reason if the iPhone is left on Automatic, since there is then no fixed "current" appearance to switch from), restoring the device's original appearance afterward. `scan --orientation both` does the same for portrait/landscape (Android `settings put system accelerometer_rotation`/`user_rotation`; iOS through the scanning harness's `XCUIDevice.shared.orientation`, the same call for the Simulator and a physical iPhone). On Android it restores the device's exact original orientation and rotation-lock state afterward; on iOS there is no way to read the original orientation back, so it rotates back to whichever of portrait/landscape the first capture showed. Without either flag, only the device's current appearance and orientation are checked.
...
- **Check manually:** Watch each screen for at least 10-15 seconds with no input and check any moving, blinking, scrolling or auto-updating content for a way to pause, stop or hide it.
...
- **Check manually:** For a text field flagged by missing-name, check whether its visible label is wired up with android:labelFor before treating it as unlabelled; confirm with TalkBack that the label is (or isn't) announced.

### A name set inside a Jetpack Compose button can land on a different uiautomator node than its clickable one

`android-compose-merged-name` · Limitation · Rules · rules: `label-in-name`, `identifier-name`, `atf`, `screen-reader-label-in-name`

- **What:** Verified 2026-09-23 with samples/NativeAndroid's Compose screen: for several IconButtons/Buttons whose accessible name is set with contentDescription on a child Icon() or with Modifier.semantics { contentDescription = ... } on the button itself, uiautomator dump's clickable="true", focusable="true" node reports an empty content-desc, and the name appears only on a separate, non-clickable child node (confirmed in the raw dump; bounds and the name are both present, just on a different node). One IconButton on the same screen, built the same way but with contentDescription = null, correctly showed up as an unlabeled clickable button, and a TextField's label = { ... } parameter correctly named the field -- so this isn't every Compose control, and the exact condition that causes the split hasn't been pinned down. Partly addressed 2026-09-23 for the unambiguous case: when exactly one non-focusable descendant carries a name and nothing else in the subtree is independently focusable or clickable, UiAutomatorParser.TryMergeDescendantName gives the clickable node that name as its own Label, approximating how a screen reader is expected to read the merged node together (matching the existing ScreenReaderPredictor.AccessibleName fallback, but now also seen by rules that read AccessibilityNode.Label directly). Extended 2026-09-26 for one several-named-descendants shape: exactly one descendant carrying only a content-desc, and exactly one other carrying only plain visible text, nothing else named in the subtree. The one real button confirmed with this shape is samples/NativeAndroid's N5 -- Modifier.semantics { contentDescription = "Submit" } set directly on the Button itself (not on an icon; N5 has no icon), with a separate Text("Pay") child -- and real TalkBack captures of that exact button on both the Android emulator (TalkBack 16.0.0, 2026-09-23, re-confirmed in two separate device sessions on 2026-09-26) and a physical Pixel 4a (TalkBack 17.0.1, two separate device sessions on 2026-09-26) announce BOTH parts, in the same order (content-desc, then visible text, then role) every time -- "Submit || Pay || Button" on the emulator (three separate utterances, joined by Swipewalk's own harness) and "Submit. Pay. Button." on the Pixel (TalkBack 17 sent it as one combined utterance with sentence-final periods, which this comparator already parses the same way -- see ScreenReaderCaptureComparer.TryStripTrailingWord's ". " separator; see also docs/case-study.md "Real TalkBack capture, in five languages"). The parser now merges this tree shape wherever it occurs (the same shape can occur on classic Android Views, not only Compose, though only this one Compose button has real TalkBack evidence behind it): the clickable node's Label becomes the full joined name ("Submit, Pay") and its VisibleText becomes the text part ("Pay"), which is what lets label-in-name evaluate the node at all -- though by that same construction the merged name always contains the merged VisibleText, so label-in-name can never report a mismatch for this exact shape either way (see Impact). Still deliberately NOT merged: a several-named-descendants shape with more than one contentDescription candidate, more than one plain-text candidate, or a descendant that carries both its own content-desc and its own visible text -- which one (or in what order) would actually be announced isn't established for those shapes, so this still doesn't guess.
- **Impact:** identifier-name now reports the clickable element's real role (e.g. "button" instead of "group") for the single-named-descendant case. This role fix does NOT extend to the new N5 (one contentDescription + one text descendant) shape: the content-desc descendant's own Label is deliberately left in place there (not cleared), so that a developer-identifier name on it is still caught by identifier-name -- with the pre-existing wrong (non-clickable) role, same as before this change, rather than silenced entirely (clearing it would have erased the finding, since a name containing the required ", " join never looks like a developer identifier). Untested on a real screen either way, since neither N5's "Submit" nor "Pay" looks like a developer identifier. missing-name's own result is unchanged -- it already found these buttons named through ScreenReaderPredictor.AccessibleName's descendant-walk fallback, so the model-level fix only makes that more robust for other rules, not a behavior change for missing-name itself. Corrected assumption: target-size was never blocked by this gap -- it already reads the clickable node's own (correct) bounds regardless of naming; a measured 48x48dp Compose IconButton not tripping target-size is IconButton's own minimum-touch-target padding (similar to samples/BuggyApp's B5, though B5 enlarges to only 44dp and still trips the platform advisory), not this naming gap. Changed for N5's exact shape: label-in-name now evaluates samples/NativeAndroid's N5 button and reports no mismatch, consistent with real TalkBack capture on both the emulator and a physical Pixel 4a on 2026-09-26, in two separate device sessions (this change's own, and the TalkBack-completeness fix's below) -- confirmed live on both devices both times, not only from the saved fixture. That is TalkBack evidence, not proof of a WCAG 2.5.3 pass: whether Android Voice Access would actually activate N5's button by saying "Pay" hasn't been tested. This also means the tree alone can no longer show a real 2.5.3 mismatch for this specific shape (a content-desc descendant plus a separate text descendant, nothing else named): by construction the merged name always contains the merged visible text, so a genuine override that hides the visible text from screen readers -- the case label-in-name exists to catch -- would need a different shape (for example the button's own content-desc set directly, not merged from a child) to be caught. Still open: a several-named-descendants button with more than one contentDescription candidate, more than one text candidate, or a descendant carrying both its own content-desc and visible text, is still left entirely unlabeled (Label and VisibleText both null), and identifier-name still reports the wrong (non-clickable) role for those shapes, since nothing is merged there. Google's Accessibility Test Framework's ImageContrastCheck also reported nothing for a low-contrast icon built the same way, for a reason not determined. With --screen-reader, screen-reader-label-in-name (added 2026-09-25) adds independent, capture-based coverage on top of the tree-only merge for two of the several-named-descendants shapes: the one now merged (through the node's own VisibleText) and an unmerged shape with exactly one visible-text candidate (through ScreenReaderLabelInNameRule.SingleDescendantVisibleText) -- it compares a control's visible text against what TalkBack actually said, rather than assuming the tree's merged name is correct, so a genuine mismatch the parser's construction can't reveal would still surface there. It does NOT cover a shape with more than one visible-text candidate, which SingleDescendantVisibleText also declines as ambiguous. This check runs whenever a capture reaches every focusable or interactive element it found and TalkBack said something for each (see "android-screen-reader-capture") -- fixed 2026-09-26 (ruleset 2026.09.28): TalkBackCollector.kt previously reported every capture as incomplete regardless of outcome, so this check could never fire on real device data before that fix; the N5 captures above are the first to meet that condition, on both devices.
- **Check manually:** For a Compose or Views button whose visible text and accessible name might differ across separate descendants -- merged by the parser or not -- check with TalkBack what is actually announced, and with speech input (Voice Access) whether saying the visible text activates it; the tree-only checks can never show a mismatch for the one shape this parser merges, so this manual check matters most there.

### TalkBack capture is opt-in, checks only some of the screen, and silences the phone while it runs

`android-screen-reader-capture` · Limitation · ScreenReader · rules: `screen-reader-capture`, `screen-reader-label-in-name`
...
- **Check manually:** On a screen with a scrollable region, also check by hand at 200% text size (Android) or your platform's largest text size whether every control present at normal size can still be reached, especially outside that scrollable area.

## iOS · All frameworks

### On a physical iPhone the large-text check drives the Settings app

`ios-large-text-physical-settings` · Limitation · Coverage
...
### The appearance rescan is skipped on a physical iPhone left on Automatic

`appearance-physical-iphone-automatic` · Limitation · Coverage

- **What:** scan --appearance both works on a physical iPhone by driving Settings > Appearance (reading the current choice, switching to the other explicit appearance, then restoring it), the same as the iOS Simulator (`simctl ui appearance`) and Android (`cmd uimode night`). A device set to Automatic (day/night scheduling) has no single "current" appearance to switch from, so that case is left untouched and the check is skipped with a reason rather than guessed. Verified on iOS 27 (where the Light/Dark picker lives on its own "Appearance" screen, separate from Display & Brightness); on another iOS version the Settings layout may differ, and the check is skipped there with a reason too rather than driving the wrong screen.
- **Impact:** On a physical iPhone set to Automatic, only whichever appearance the device happened to be showing is checked; a contrast failure that only shows up in the other appearance is missed unless the phone is set to Light or Dark by hand and scanned again in each.
- **Check manually:** On a physical iPhone, set Settings > Appearance to Light or Dark (instead of Automatic) and scan again in each.

### The orientation rescan can't tell a physical iPhone's rotation lock from a genuinely restricted screen

`orientation-physical-iphone-rotation-lock` · Limitation · Coverage

- **What:** scan --orientation both rotates a physical iPhone the same way as the iOS Simulator, through the scanning harness's XCUIDevice.shared.orientation (there is no `simctl` equivalent). On a physical device this is a simulated sensor event: if Control Center's rotation lock is on, the call can report success while the interface never actually turns. There is no public API here to read the lock's own state, so Swipewalk shows a heads-up before rotating a physical iPhone, but cannot detect the lock itself -- a screen that looks the same after rotating is reported for review the same way a genuinely orientation-restricted screen is (never as a confirmed WCAG failure either way).
- **Impact:** On a physical iPhone with rotation lock on, a screen that actually supports both orientations can be flagged for review as if it were restricted to one.
- **Check manually:** Before scanning a physical iPhone with --orientation both, check Control Center and turn rotation lock off. If a screen is flagged, rotate the device by hand (with rotation lock off) to confirm whether it's a real restriction.

### iOS overlays over the app may be captured while recording

`ios-overlays-during-recording` · Limitation · Detection

- **What:** Record mode captures only while the app under test is in front, but iOS can report the app as in front while Notification Center or Control Center is pulled down over it.
- **Impact:** A screenshot taken at that moment could include notifications.
- **Check manually:** Don't open Notification Center or Control Center while recording, and review screenshots before sharing reports.

### A device .ipa must be signed for that device to be installed

`ios-install-unsigned-ipa` · Limitation · Devices

- **What:** Any installed app can be scanned, whoever signed it. But iOS only installs an .ipa file signed for the device: a development or ad-hoc build whose profile includes it, or an enterprise build. App Store .ipa files cannot be sideloaded, and Swipewalk does not re-sign apps.
- **Impact:** Builds signed for other devices, or App Store builds from a file, cannot be installed and scanned from the file.
- **Check manually:** Install the app through TestFlight, the App Store or your MDM and scan it by --bundle-id, or scan a Simulator build.
- **Planned:** Installing builds signed for other devices

### Physical iPhones: slower recording, some checks unverified

`ios-physical-devices` · Limitation · Devices

- **What:** scan and record work on physical iPhones and iPads (tested on an iPhone with iOS 18.6). The harness is signed with a fitting installed development profile (no Apple ID needed) or Xcode automatic signing, and exchanges files through devicectl. Each check of the screen takes several seconds, so new screens are noticed with a delay. The first run asks for Face ID or the passcode to allow UI automation. Manual signing with a hand-installed profile, and the pause when another app comes to the front during recording, are covered by tests but not yet confirmed on hardware. Bringing the app to the front (or starting it) when a scan or recording begins was verified on the Simulator only.
...
- **Check manually:** Swipe through with VoiceOver and check the order is logical.

### The Accessibility Inspector route is opt-in, needs a person present, and covers scan only for now

`ios-inspector-walk-capture` · Limitation · ScreenReader · rules: `screen-reader-capture`
...
- **Check manually:** Swipe through with VoiceOver directly for anything the Inspector route was declined for, only partly captured, or matched with Weak confidence -- and for swipe order generally, since this route doesn't check it at all.
...
- **Check manually:** For an icon-only control, check with VoiceOver that the announced name actually describes what the control does here, not just that a name exists.

### SwiftUI's .accessibilityLabel() replaces a button's visible text in the captured tree

`ios-swiftui-accessibility-label-hides-text` · Limitation · Rules · rules: `label-in-name`

- **What:** Verified 2026-09-23 with samples/NativeiOS: a SwiftUI Button("Pay") { }.accessibilityLabel("Submit") is captured with accessible name "Submit" and no trace of the visible text "Pay" anywhere in the tree. The equivalent UIKit button (a title "Pay" plus accessibilityLabel = "Submit") keeps "Pay" as a separate child node alongside the button's own "Submit" label, which is what label-in-name compares against.
...
- **Check manually:** For a SwiftUI button, compare its visible text against what VoiceOver announces directly, rather than relying on label-in-name to catch a mismatch.

## Windows · All frameworks

### Windows apps can't be scanned yet

`windows-not-supported` · Limitation · Devices

- **What:** The Windows collector (UI Automation with axe-windows) is not implemented.
- **Impact:** No automated results for Windows.
- **Check manually:** Test with Narrator and keyboard only, or use Accessibility Insights for Windows.
- **Planned:** Windows support

## Android · .NET MAUI

### MAUI tap gestures on labels are invisible on Android

`maui-android-tap-gesture` · Limitation · Detection

- **What:** A MAUI Label (or other view) with a TapGestureRecognizer is not marked clickable in Android's accessibility tree, so the scanner doesn't see it as a control.
...
- **Check manually:** Change the text size while the app is running and check whether it takes effect without a restart; if not, relaunch and check text at the new size for clipping. Check the app's Microsoft.Maui.Controls version against 10.0.100.

### MAUI may enlarge small ImageButtons at runtime

`maui-imagebutton-touch-area` · Framework note · Rules · rules: `target-size`

- **What:** An ImageButton declared 20×20 was measured 44×44 on screen on both Android and iOS, because the button is enlarged at runtime. The scanner uses the measured size, not the declared one.
...
- **Check manually:** Where the XAML size is below 24×24, tap near the edge of the reported area to confirm the enlarged area actually responds.

## All platforms · Other / native

### Fix examples are for MAUI and native APIs only

`other-framework-fix-examples` · Limitation · FixExamples

- **What:** Scanning should work for any framework that exposes its UI to the operating system's accessibility layer, but only .NET MAUI and native apps have been tested. Fix examples exist only for .NET MAUI and native Android/iOS; Flutter, React Native, Jetpack Compose and SwiftUI get native or generic advice.
- **Impact:** The code example may not match the app's framework.
- **Check manually:** Apply the same fix with the framework's accessibility API (for example Semantics in Flutter, accessibilityLabel in React Native).
