...
`android-compose-merged-name` · Limitation · Rules · rules: `label-in-name`, `identifier-name`, `atf`
...
- **Check manually:** For a Compose button whose visible text and accessible name might differ (an icon plus separate text under one merged control), check with a screen reader what is actually announced, rather than trusting a clean Swipewalk report.

### TalkBack capture is opt-in, checks only some of the screen, and silences the phone while it runs

`android-screen-reader-capture` · Limitation · ScreenReader · rules: `screen-reader-capture`
