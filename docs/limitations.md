...
### Only the spacing exception of 2.5.8 is evaluated

`target-size-exceptions` · Limitation · Rules · rules: `target-size`

- **What:** Small targets are checked against the 24×24 minimum and the spacing exception. The equivalent-control, inline, user-agent and essential exceptions are not evaluated. dp and pt are treated as CSS pixels (an approximation).
...
### The orientation rescan doesn't support a physical iPhone yet

`orientation-physical-iphone` · Limitation · Coverage

- **What:** scan --orientation both works on the iOS Simulator (via the scanning harness's XCUIDevice.shared.orientation -- there is no `simctl` equivalent) and on Android, real device or emulator (via `settings put system accelerometer_rotation`/`user_rotation`), but Swipewalk doesn't rotate a physical iPhone yet, so the check is skipped there with a reason.
...
- **Check manually:** On a physical iPhone, turn off rotation lock, rotate the device by hand, and scan again to check whether the screen followed.
...
- **Impact:** Reachability can be predicted wrongly for custom views, so missing heading and control roles (1.3.1, 4.1.2) are not detected except where Apple's audit reports them, or where the Inspector route flags one of the small set of roles above as unconfirmed for review.
...
### VoiceOver-captions capture (--voiceover-captions) is a draft, not yet verified on a real device

`ios-voiceover-captions-capture` · Limitation · ScreenReader · rules: `screen-reader-capture`

- **What:** With --voiceover-captions on `record`, a person turns VoiceOver and its Caption Panel (Settings > Accessibility > VoiceOver > Caption Panel) on themselves and swipes through the screen, while this Mac polls the phone's screen over the cable (`xcrun devicectl device capture screenshot`, no XCUITest, no extra permission) and reads the on-screen caption text with Vision's on-device text recognition (harness/mac-voiceover-captions/VoiceOverCaptionCapture.swift). VoiceOver is never scripted or turned on by Swipewalk. Consecutive identical captions are treated as one utterance; a caption disappearing and reappearing (the person swiped back to something they already heard) is treated as a new one. Each utterance is matched to a scanned tree node by a best-effort, unverified detection of VoiceOver's own on-screen cursor rectangle (overlap with the node's bounds), then by the caption text containing exactly one candidate's predicted accessible name -- both confirmed only against synthetic fixtures so far (drawn caption bars and cursor strokes on a plain background), never a real Caption Panel or a real VoiceOver cursor. The capture is never reported complete: unlike a scripted walk, nothing here can tell whether the person swiped through the whole screen or stopped partway, so a predicted stop this didn't capture is never reported as Missing (see ScreenReaderCaptureRule). Unlike TalkBack's and the Accessibility Inspector's captures, an order difference from this source IS reported (1.3.2, 2.4.3): a person's real swipe order should be the reader's genuine navigation order, but it is not yet confirmed on hardware that a person naturally starts from the very first element on the screen -- if they don't, the same rotation-looks-like-a-difference risk the Inspector route has applies here too. Also unconfirmed on hardware: the caption panel's timing (how long after an announcement the caption updates, and for how long it stays), OCR accuracy on a real Caption Panel's font/rendering in more than one language, and how a screen's other dark UI (a tab bar, a status/home indicator) affects the caption-region finder -- one real-device screenshot check (2026-09-26, no VoiceOver session, just proving the screenshot mechanism) found the iPhone's home-indicator bar can be mistaken for the caption region by the region-finder, though it produced no caption text there (Vision found nothing to read) so no false utterance resulted; a real caption session could differ.
- **Impact:** Off by default, record only, and not yet proven against a real VoiceOver session -- treat any evidence from this route as a first look to confirm by eye, not settled fact, until it has been run against real hardware. A missed or wrongly-matched utterance is possible; the focus-rect detection especially is a coarse heuristic.
- **Check manually:** Swipe through with VoiceOver directly and compare by ear against the predicted transcript and whatever this route reported.
- **Planned:** A real-device verification session (English first, then Spanish and a non-Latin script); tightening the caption-region and focus-rect detectors against real Caption Panel screenshots; deciding whether the poll interval needs adjusting once real caption timing is known.
