...
| `orientation-restricted` | Whether the screen's content visibly follows the device being rotated between portrait and landscape (scan --orientation both; for review, since a single orientation can be essential to a screen) | Needs review (the screen still looked the same shape after the device was rotated -- always for review, never a confirmed failure, since a single orientation can be essential to a screen) | 1.3.4 Orientation (AA) | Android; iOS (Simulator only) |
