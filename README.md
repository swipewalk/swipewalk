...
Its own rules cover missing accessible names, touch target size, identifier-like names, visible-text-in-name, text contrast, text resize, controls or text that disappear at large text sizes with no scrollable area to reach them (for review), controls or text positioned wholly or mostly off-screen at normal text size with no scrollable area to reach them (for review; iOS), whether Android screens expose a pane title, candidate personal-data input fields with no confirmed autofill/content-type hint, and icon contrast (iOS), plus the platform engines it wraps (Apple's accessibility audit on iOS, Google's Accessibility Test Framework on Android) — see [every check, what it looks for and its WCAG mapping](docs/checks.md). `scan --appearance both` also captures and checks the screen in the device's other dark/light appearance, so a contrast failure that only shows up in one theme isn't missed just because the device happened to be in the other one. `scan --orientation both` rotates the device between portrait and landscape and flags a screen for review (WCAG 1.3.4 Orientation) when its content doesn't visibly follow.
...
Every report also lists all 55 WCAG 2.2 A/AA success criteria, with what this scan's automated checks did for each one and how to check the rest by hand — see [the user guide](docs/user-guide.md#wcag-22-coverage); on a screen captured with `--screen-reader`, that coverage also names real evidence (TalkBack's speech, or the Accessibility Inspector's traits) for the criteria it exercises. `swipewalk guide <run>` (and the desktop app's Guided checks page) walks a saved run's screens with step-by-step TalkBack/VoiceOver questions, records the tester's answer with required evidence for a pass, and flags it when a recorded pass disagrees with an automated finding — see [Guided checks](docs/user-guide.md#6-guided-checks).
Findings are labelled with the [laws and standards](docs/standards.md) whose WCAG version and level include them (ADA Title II, Section 508, EN 301 549 v3.2.1 and v4.1.1, UK public sector regulations). That is a mapping to help prioritise, not legal advice. Each report records the rule versions it was checked against and warns when a mapping was last reviewed more than a year earlier. US states and other countries are researched too — see [docs/standards.md](docs/standards.md) for what's confirmed, and pass `--standard <id>` to add one to a report (they're opt-in, so a report never lists every state by default).
...
Swipewalk never needs an app's source code and never rebuilds or re-signs it: any installed app can be scanned, debug or production, whoever signed it. `--install <file>` installs a build first: an Android `.apk` or App Bundle `.aab` (the `.aab` via Google's bundletool, usually found automatically or given with `--bundletool`), an iOS Simulator `.app`, or a device `.ipa` signed for that device (iOS cannot install an `.ipa` signed for other devices; install those through TestFlight, the App Store or MDM and scan by `--bundle-id`). See the [user guide](docs/user-guide.md#9-troubleshooting) for the `.aab`/bundletool details and the [limitations](docs/limitations.md) for apps that block screenshots or automation.
