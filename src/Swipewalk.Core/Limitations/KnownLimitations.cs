...
public static class KnownLimitations
{
    ...
    [
        ...
        new()
        {
            Id = "appearance-physical-iphone-automatic",
            Area = LimitationArea.Coverage,
            Platforms = [Platform.iOS],
            Title = "The appearance rescan is skipped on a physical iPhone left on Automatic",
            Description = "scan --appearance both works on a physical iPhone by driving Settings > Appearance (reading the current choice, switching to the other explicit appearance, then restoring it), the same as the iOS Simulator (`simctl ui appearance`) and Android (`cmd uimode night`). A device set to Automatic (day/night scheduling) has no single \"current\" appearance to switch from, so that case is left untouched and the check is skipped with a reason rather than guessed. Verified on iOS 27 (where the Light/Dark picker lives on its own \"Appearance\" screen, separate from Display & Brightness); on another iOS version the Settings layout may differ, and the check is skipped there with a reason too rather than driving the wrong screen.",
            ...
            ManualCheck = "On a physical iPhone, set Settings > Appearance to Light or Dark (instead of Automatic) and scan again in each.",
        },
        new()
        {
            Id = "orientation-physical-iphone-rotation-lock",
            Area = LimitationArea.Coverage,
            Platforms = [Platform.iOS],
            Title = "The orientation rescan can't tell a physical iPhone's rotation lock from a genuinely restricted screen",
            Description = "scan --orientation both rotates a physical iPhone the same way as the iOS Simulator, through the scanning harness's XCUIDevice.shared.orientation (there is no `simctl` equivalent). On a physical device this is a simulated sensor event: if Control Center's rotation lock is on, the call can report success while the interface never actually turns. There is no public API here to read the lock's own state, so Swipewalk shows a heads-up before rotating a physical iPhone, but cannot detect the lock itself -- a screen that looks the same after rotating is reported for review the same way a genuinely orientation-restricted screen is (never as a confirmed WCAG failure either way).",
            ...
            ManualCheck = "Before scanning a physical iPhone with --orientation both, check Control Center and turn rotation lock off. If a screen is flagged, rotate the device by hand (with rotation lock off) to confirm whether it's a real restriction.",
        },
        new()
        {
